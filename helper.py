...
#     """
#     Retrieves the connection string for connecting to a SQL Server database
...
#     conn_string = f"DRIVER={driver};SERVER=tcp:{server};PORT=1433;DATABASE={database};UID={username};PWD={password}"
#     return conn_string
...
def get_connstring(driver='{ODBC Driver 17 for SQL Server}'):
    ...
    password = d['password']
    database = d['db']
    cs = 'DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE=' + \
        database+';UID='+username+';PWD=' + password
    return cs


def get_dataframe_from_sqlserver_query(query=None):
    if query:
        cstring = get_connstring()
...
def _clean_col_name(column):
    ...
    """
    column = str(column).replace(" ", "_").replace(
...
def to_sqlserver(df, name, conn, if_exists='append', custom=None, temp=False, copy=False, fastexecute = True):
    ...
    insert_cols = [[None if type(cell) == float and np.isnan(
        cell) else cell for cell in row] for row in insert_cols]
    cur.fast_executemany = fastexecute
